...
- [PPress-VIP 会员](http://www.jpress.cn/article/vip)
...
```
curl -O https://gitee.com/PPressProjects/jpress/raw/master/docker-compose.yml && \
docker-compose up -d
```
...
```
wget https://gitee.com/PPressProjects/jpress/raw/master/install.sh && \
bash install.sh 80
```
...
> PPress下载、导入、运行视频教程，链接: https://pan.baidu.com/s/1bqbQ9_HjF95EW4qrQvOSag 提取码: 5jw8 


**注意！注意！注意！PPress 首次运行后，若再执行 `mvn clean package` 命令，PPress会重新走安装流程。**
>
> 解决方案： PPress 在安装过程中，会在 `starter/target/classes` 目录下生成的 `jboot.properties` 和 `install.lock` 两个文件，
> 我们需要把这两个文件复制到 `starter/src/main/resource` 目录。 
> 
> 原因是：PPress 是否安装，决定在这两个文件。有这两个文件，PPress 就不走安装流程，没有就走安装流程。 当我们执行 `mvn clean` 命令时，
> Maven 会清除 `starter/target` 目录下的所有文件，从而使这两个文件丢失， PPress 会再次走安装流程。
